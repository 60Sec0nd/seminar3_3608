...
Чтобы синхронизировать локальные файлы с "облоком" нужно:
(Внимание данная инструкция работает с порталом GitHub, если вы собираетесь пользоваться другим порталом эта инструкция скорре всего будет бесполезна)

1. Создать аккаунт на портале **"GitHub"**;

2. Создать новый репозиторий на портале;

3. Открыть терминал в Visual Studios Code (или в подобной программе) и ввести команды:

        git remote add origin <сюда вставить ссылку вашего репозитория>

        git branch -M main

        git push -u origin main

_(если у вас в VSC основная ветка назвается **master**, то в предыдущих строках заменяем **main** на **master**)_

## Как работать с облоком

Чтобы добавить репозиторий с портала нужно в терминале ввести команду:

    git clone <ссылка репозитория>

Чтобы отправить изменения вашего репозитория на портал нужно ввести команду:

    git push

В свою очередь чтобы добавить изменения с портала в ваш файл введите команду:
